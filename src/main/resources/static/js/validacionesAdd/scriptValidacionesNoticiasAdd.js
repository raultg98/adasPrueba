...
descripcion.addEventListener('change', (e) => {
    ...
    if(contenidoDescripcion.length > 5){
        ...
        crearMensajeError('La descripcion es demasiado corta');
...
imagen.addEventListener('change', (e) => {
    ...
    if(!expReg.test(extensionArchivo)){
        crearMensajeError('La extensión del archivo no es válida (png/jpeg/jpg), '+ extensionArchivo);
    ...
    reader.onload = function(){
        const img = document.getElementById('noticiaMostrarNuevaFoto');
...
formulario.addEventListener('change', (e) => {
    if(validaciones.titulo === 'true' && validaciones.descripcion === 'true' && validaciones.imagen === 'true' && validaciones.usuario === 'true'){
